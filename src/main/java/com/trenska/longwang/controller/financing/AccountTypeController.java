...
public class AccountTypeController {
	...
	public ResponseModel addReceiptAccountType(String typeName) {

		if(null == typeName) return ResponseModel.getInstance().succ(false).msg("账目类型不能为空");
	...
	public ResponseModel addPayAccountType(String typeName) {
	...
	@ApiOperation("删除账目类型")
	public ResponseModel deleteAccountType(@PathVariable Integer id) {
		if(!NumberUtil.isIntegerUsable(id)){
			return ResponseModel.getInstance().succ(false).msg("无此账目类型");
		}
		...
		if(!accountType.getDeletable()){
			return ResponseModel.getInstance().succ(false).msg("系统预留，不可删除");
		}
		return ResponseModel.getInstance().succ(accountTypeService.removeById(id))
