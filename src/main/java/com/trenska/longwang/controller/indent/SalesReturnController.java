...
public class SalesReturnController {
	...
	@ApiOperation("添加退货单")
	public CommonResponse addSalesReturn(@RequestBody Indent indent) {
		if (null == indent) {
			return CommonResponse.getInstance().succ(false).msg("无此退货单.");
		}
	...
	@ApiOperation("作废退货单")
	public CommonResponse invalid(@PathVariable("indentId") Integer indentId) {
		Indent indent = indentService.getById(indentId);
		if (indent == null) {
			return CommonResponse.getInstance().succ(false).msg("不存在该退货单信息");
		} else if (!IndentStat.FINISHED.getName().equals(indent.getStat())) {
			return CommonResponse.getInstance().succ(false).msg("已完成的退货单才能作废");
		}
	...
	public CommonResponse deleteIndentById(@ApiParam(name = "indentId", required = true) @PathVariable("indentId") Long indentId) {
		if (!NumberUtil.isLongUsable(indentId)) {
			return CommonResponse.getInstance().succ(false).msg("删除退货单失败 : 不存在此退货单");
		}
	...
	public CommonResponse batchDeleteReturnSales(@ApiParam(name = "indentIds", value = "需要批量删除的退货单编号集合/数组", required = true) @RequestParam(value = "indentIds") Collection<Long> indentIds) {
		if (indentIds == null || indentIds.isEmpty()) {
			return CommonResponse.getInstance().succ(false).msg("删除退货单失败 : 包含不存在的退货单");
		}
