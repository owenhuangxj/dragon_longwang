...
public class ReceiptUtil {
	...
	public static String getReceiptNo(String prefix, String type, ReceiptMapper receiptMapper) {
		...
		String currentDateStr = new SimpleDateFormat(Constant.DATE_FORMAT).format(new Date());
		String dbReceiptNo;
		...
		if (isLastReceiptRecordNull || !isTwoDateEquals) {
			...
			Integer num = BillsUtil.getSerialNumber(Optional.of(receiptNo)) + 1;
			// 通过最后一条的库存单号生成新的库存单号
	...
	public static ResponseModel saveReceipt(Receipt receipt, HttpServletRequest request, ReceiptMapper receiptMapper, String billType, String receiptType) {
		...
		Integer startNumber = BillsUtil.getSerialNumber(Optional.of(receiptNo));
		// 收/付款时间
		...
		for (ReceiptModel receiptModel : receiptSet) {
			...
			DealDetailUtil.saveDealDetail(custId, nameNo, currentTime, amount, newDebt.toString(), oper, payway, remarks);
			startNumber++;
