...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements ICategoryService {
	...
	public Boolean removeCategoryById(Integer categoryId) {
		...
		Set<Integer> categoryIds = subCategories.stream().map(Category::getCatId).collect(Collectors.toSet());
	...
	private List<Category> findSubCategoriesByPid(Integer pid) {
		List<Category> categories = super.baseMapper.selectList(
				new LambdaQueryWrapper<Category>().eq(Category::getPid, pid)
		);
		return categories;
	}
}
