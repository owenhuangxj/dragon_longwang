...
public class ActiveServiceImpl extends ServiceImpl<ActiveMapper, Active> implements IActiveService {
	...
	@Override
	@Transactional
	public CommonResponse saveActive(Active active) {
		// 首先保存活动数据到t_active
		this.save(active);
...
//			activeAreaGrpMapper.insert(new ActiveAreaGrp(activeId, areaGrpId));
		...
		return CommonResponse.getInstance().succ(true).msg("创建商品活动成功.");
	}

	@Override
	@Transactional
	public CommonResponse removeActiveById(Integer activeId) {
		...
		if (active.getStat()) {
			return CommonResponse.getInstance().succ(false).msg("不能删除未结束的活动.");
		}
		...
		return CommonResponse.getInstance().succ(true).msg("删除商品活动成功.");
	}

	@Override
	public CommonResponse removeActiveByIds(Collection<Integer> activeIds) {
		/* 可删除活动数量 */
		...
		if (activeIds.size() != deletableActiveCount) {
			return CommonResponse.getInstance().succ(false).msg("你选择了未结束的活动或活动中包含无效的活动.");
		}
